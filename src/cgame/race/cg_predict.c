...
bool Cg_UsePrediction(void) {
  ...
  if (Cg_Module_DisablePrediction()) {
    return false;
  }
...
static cm_trace_t Cg_PredictMovement_Trace(const vec3_t start, const vec3_t end, const box3_t bounds) {
  return Cg_Module_TracePrediction(start, end, bounds);
}
...
void Cg_PredictMovement(const Vector *cmds) {
  ...
  for (uint32_t i = 0; i < cmds->count; i++) {
    ...
    if (cmd->cmd.msec) { // if the command has time, run it
      ...
      pm.cmd = cmd->cmd;
      Cg_Module_PreparePredictionCommand(&pm, i, cmds->count);
      Pm_Move(&pm);
      Cg_Module_CompletePredictionCommand(&pm, i, cmds->count);
    }
