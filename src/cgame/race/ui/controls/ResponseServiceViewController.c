...
static void enumerateCrosshairs(const char *path, void *data) {
  ...
  $((Select *) data, addOption, va("Crosshair %" PRIdPTR, value), (ident) value);
}

/**
 * @brief Numeric sorting, with Off first.
 */
static Order sortCrosshairs(const ident a, const ident b) {

  const intptr_t c = (intptr_t) ((const Option *) a)->value;
  const intptr_t d = (intptr_t) ((const Option *) b)->value;

  if (c == d) {
    return OrderSame;
  }
  return c < d ? OrderAscending : OrderDescending;
}
...
static void refreshBindings(ViewController *self) {
...
static void didBindKey(TextView *textView) {
  ...
  ViewController *this = textView->delegate.self;
...
static void loadView(ViewController *self) {
  ...
  this->crosshair->comparator = sortCrosshairs;

  $(this->crosshair, addOption, "Off", (ident) 0);
