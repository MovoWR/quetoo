...
static uint32_t G_Ai_Move(g_client_t *cl, pm_cmd_t *cmd) {
  ...
  if (cl->ai->move_target.type == AI_GOAL_PATH) {
    ...
    // otherwise hold jump
      ...
      if ((cl->ai->move_target.path.path_position.z - ent->s.origin.z) < -Box3_Size(Pm_PlayerBounds(false)).z) {
  ...
  if (combat_target->type != AI_GOAL_ENTITY) {
    if (cl->ai->move_target.type == AI_GOAL_NONE) {
      ...
      if (cl->ps.pm_state.flags & PMF_ON_LADDER) {
        if ((cl->ai->move_target.path.path_position.z - cl->entity->s.origin.z) < -Box3_Size(Pm_PlayerBounds(false)).z) {
...
void G_Ai_Frame(void) {
  ...
  if (g_level.time % 1000 == 0) {
    ...
    const int32_t base_min_clients = g_level.min_clients_map > -1 ? g_level.min_clients_map : sv_min_clients->integer;
    const int32_t min_clients = Maxi(0, base_min_clients);
