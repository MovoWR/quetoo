...
static void G_Use_f(g_client_t *cl) {
  ...
  if (it->def.type == ITEM_TYPE_WEAPON && !G_Module_WeaponsEnabled()) {
    gi.ClientPrint(cl, PRINT_HIGH, "Weapons are disabled on this map\n");
    return;
  }
 ...
 */
static void G_ClientKill_Common(g_client_t *cl) {
...
ClientKill G_ClientKill = G_ClientKill_Common;
...
static void G_Spectate_f(g_client_t *cl) {
  ...
  if (G_Module_ClientModeChange(cl, q_strcmp(gi.Argv(0), "spectate") == 0)) {
    return;
  }
...
void G_ClientCommand(g_client_t *cl) {
  ...
  if (g_level.intermission_time) {
    G_Module_IntermissionClientCommand(cl, cmd);
    return;
  }
  ...
  if (q_strcmp(cmd, "spectate") == 0 || q_strcmp(cmd, "join") == 0) {
    ...
    G_God_f(cl);
  } else if (q_strcmp(cmd, "no_clip") == 0 || q_strcmp(cmd, "noclip") == 0) {
    G_NoClip_f(cl);
