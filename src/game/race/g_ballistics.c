...
static void G_BlasterProjectile_Touch(g_entity_t *ent, g_entity_t *other, const cm_trace_t *trace) {
  ...
  if (!G_IsSky(trace)) {
    ...
    G_Damage(&(g_damage_t) {
      ...
      .flags = DMG_ENERGY,
      .mod = ent->mod ? ent->mod : MOD_BLASTER
    });
...
static void G_NailProjectile_Touch(g_entity_t *ent, g_entity_t *other, const cm_trace_t *trace) {
  ...
  if (!G_IsSky(trace)) {
    ...
    G_Damage(&(g_damage_t) {
      ...
      .damage = ent->damage,
      .knockback = ent->knockback,
      .mod = ent->mod ? ent->mod : MOD_QUAKE_NAILGUN
    });
...
static void G_RocketProjectile_Touch(g_entity_t *ent, g_entity_t *other, const cm_trace_t *trace) {
  ...
  const bool observed_rocket = (ent->spawn_flags & OBSERVED_ROCKET) != 0;
  const uint32_t direct_mod = ent->mod ? ent->mod : (quake_rocket ? MOD_QUAKE_ROCKET : MOD_ROCKET);
  const uint32_t splash_mod = ent->mod ? ent->mod : (quake_rocket ? MOD_QUAKE_ROCKET_SPLASH : MOD_ROCKET_SPLASH);
  ...
  const bool impact = !G_IsSky(trace) &&
                      (G_IsStructural(trace) || G_IsMeat(other));
  if (impact) {
  ...
  if (observed_rocket) {
    G_ObserveProjectile(ent, G_PROJECTILE_ROCKET,
                        impact ? G_PROJECTILE_IMPACT
...
static void G_RocketProjectile_Expire(g_entity_t *ent) {
  G_ObserveProjectile(ent, G_PROJECTILE_ROCKET,
...
void G_RocketProjectile(g_entity_t *emitter, g_entity_t *attacker, const vec3_t start, const vec3_t dir, int32_t speed, int32_t damage, int32_t knockback, float damage_radius, uint32_t mod) {
  ...
  projectile->move_type = MOVE_TYPE_FLY;
  projectile->next_think = g_level.time + 8000;
  projectile->Think = G_RocketProjectile_Expire;
...
static void G_HyperblasterProjectile_Touch(g_entity_t *ent, g_entity_t *other, const cm_trace_t *trace) {
  ...
  G_ObserveProjectile(ent, G_PROJECTILE_HYPERBLASTER,
                      impact ? G_PROJECTILE_IMPACT
                             : G_PROJECTILE_SILENT_DESPAWN,
                      trace->plane.normal);
...
static void G_HyperblasterProjectile_Expire(g_entity_t *ent) {
  G_ObserveProjectile(ent, G_PROJECTILE_HYPERBLASTER,
                      G_PROJECTILE_SILENT_DESPAWN, Vec3_Zero());
  G_FreeEntity(ent);
}
...
void G_HyperblasterProjectile(g_entity_t *emitter, g_entity_t *attacker, const vec3_t start, const vec3_t dir, int32_t speed, int32_t damage, int32_t knockback) {
  ...
  projectile->Think = G_HyperblasterProjectile_Expire;
  ...
  if (ent->damage) { // shoot, removing our damage until it is renewed
    if (G_TakesDamage(tr.ent)) { // try to damage what we hit
      G_Damage(&(g_damage_t) {
        ...
        .knockback = ent->knockback,
        .flags = DMG_ENERGY,
        .mod = ent->mod ? ent->mod : MOD_LIGHTNING
      });
      ent->damage = 0;
