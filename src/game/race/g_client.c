...
static bool G_WouldTelefrag(const vec3_t spot) {
  ...
  box3_t bounds = Box3_Translate(Pm_PlayerBounds(false), spot);
...
static void G_ClientRespawn_(g_client_t *cl) {
  ...
  // Find the stock spawn point, then let Race finalize this bounded result.
  const g_entity_t *spawn = G_SelectSpawnPoint(cl);
  assert(spawn);

  g_client_spawn_t client_spawn = {
    .origin = spawn->s.origin,
    .angles = spawn->s.angles,
    .clip_mask = cl->ai ? CONTENTS_MASK_CLIP_MONSTER : CONTENTS_MASK_CLIP_PLAYER,
    .kill_box = true,
  };
  client_spawn.origin.z += PM_STEP_HEIGHT;

  G_Module_ClientSpawn(cl, &client_spawn);

  // Move to the finalized spawn origin.
  ent->s.origin = client_spawn.origin;
  ...
  if (!cl->persistent.spectator && !editor->value) {
    ...
    gi.WriteAngles(client_spawn.angles);
    gi.Unicast(cl, true);
  }
  ...
  if (cl->persistent.spectator || editor->value) { // spawn a spectator
    ...
    ent->bounds = Pm_PlayerBounds(false);
    ...
    ent->clip_mask = client_spawn.clip_mask;
...
void G_ClientUserInfoChanged(g_client_t *cl, const char *user_info) {
  ...
  const size_t len = q_strlen(user_info);
  ...
  if (q_strncmp(cl->persistent.net_name, name, sizeof(cl->persistent.net_name))) {
  ...
  G_Module_ClientUserInfoChanged(cl);
}
...
bool G_ClientConnect(g_client_t *cl, char *user_info) {
  ...
  if (q_strlen(g_password->string) && !cl->ai) {
    if (q_strcmp(g_password->string, InfoString_Get(user_info, "password"))) {
...
static cm_trace_t G_ClientMove_Trace(const vec3_t start, const vec3_t end, const box3_t bounds) {
  g_entity_t *self = g_level.current_entity;

  return G_Module_TraceMovement(self, start, end, bounds, self->clip_mask);
}
...
void G_ClientThink(g_client_t *cl, pm_cmd_t *cmd) {
  ...
  if (G_Module_ClientInput(cl, cmd)) {
    return;
  }

  if (!G_Module_ClientGameplay(cl)) {
    cl->old_buttons &= BUTTON_SCORE;
    cl->buttons &= BUTTON_SCORE;
    cl->latched_buttons &= BUTTON_SCORE;
    cl->cmd = (pm_cmd_t) {
      .msec = cmd->msec,
      .angles = cmd->angles,
      .buttons = cl->buttons
