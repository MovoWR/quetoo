...
void G_Damage(const g_damage_t *dmg) {
  ...
  g_entity_t *inflictor = dmg->inflictor ? dmg->inflictor : ge.entities[0];
  g_entity_t *attacker = dmg->attacker ? dmg->attacker : ge.entities[0];
  ...
  if (!G_Module_FilterDamage(target, attacker, &damage, &knockback)) {
    return;
  }
